...
void SortAwards() {
	...
	for (int i = 0; i < records - 1; i++)
	{
		...
		double currentMin = gSortedAward[i];
		int currentMinIndex = i;

		for (int j = i + 1; j < records; j++)
		{
			if (currentMin < gSortedAward[j])
			{
				currentMin = gSortedAward[j];
				currentMinIndex = j;
			}
		}
		...
		{
			gSortedAward[currentMinIndex] = gSortedAward[i];
			gSortedAward[i] = currentMin;
		}
	}
...
void DoAwardStatistics() {
	...
	// TODO 3. ===================================================
	// find the sum and average of gPlayerAward
	for (int i = 0; i < records; i++)
	{
		sum += gPlayerAward[i];
	}

	average = static_cast<double> (sum) / records;
	...
	for (int i = 0; i < gPlayerAward[i]; i++)

		sumElements += pow(gPlayerAward[i] - average, 2);
		
	std = sqrt(sumElements / records);
...
void TestLottery() {
	...
	fbuf.open("test-results.txt", ios::out);
	auto oldbuf = cout.rdbuf(&fbuf);
	DoAwardStatistics();
	SortAwards();
	cout.rdbuf(oldbuf);
	cout << "test done!" << endl;
